...
> 改 rules/ 时，用以下三个问题检验。全部通过则应纳入 rules/：

| # | 判断问题 | 通过条件 |
|---|---------|---------|
| Q1 | **消费者是谁？** | 使用 diwu-flow 的**任意目标项目**中的 AI（不只是改插件的 AI） |
| Q2 | **消费场景是什么？** | AI 在**执行任务前/中/后**需要参考的行为约束（不是一次性设计决策） |
| Q3 | **脱离插件源码是否仍有意义？** | 换成 Go 后端或 React 前端项目，这条规则的文字含义不变（只换术语） |
...
| **技能路由表（如 /diwu）** | **插件版本号同步脚本** |

> **Q3 是最关键的过滤项**：如果一条规则的内容全是 diwu-flow 专属术语（dpth/dref/dtask.json），不意味着它是"插件专属"。这些术语就是 diwu-flow 方法论的词汇——如同"P-J-A"、"三唯一"、"GWT"一样。判断依据是：**这条规则是在教 AI 怎么用这套方法**，还是在描述这套方法是怎么实现的。

### .doc/ 组织原则：两层分离

| 层 | 模型 | 产物 | 说明 |
|---|---|---|---|
| 文件结构层 | 文档类型驱动（按主题） | `.doc/` 下各文件 | 每份文件有明确的"回答什么/不回答什么"边界 |
| 索引导航层 | 读者意图（按角色/时机） | `.doc/README.md` | 按"初次了解/设计迭代/工程实现"组织阅读路径 |
| 写作风格层 | Diátaxis 象限（参考） | 每份文档内部 | Reference/Explanation/How-to 风格混用，按内容性质选择 |

- 文件结构按主题分，README.md 按读者意图组织导航
- Diátaxis 解决"文档内部怎么写"，不作为文件分类依据
- 详见 issue ssdiwu/diwu-flow-dev#24
