...
> 本文件只保留 Skills 未覆盖的独特内容。任务规划/实施/提交/子代理策略/验证要求分别见 dtask / djug / dvfy。
...
> dtask/djug 定义了并行条件与分工策略。本节定义 SubagentStart Hook 触发时自动注入的内容格式——这是 Skills 中未覆盖的交接协议。
...
> dvfy 定义了 L1-L5 证据等级和 Done 判定矩阵。本节定义「选择哪种验证方式」的分层决策。
