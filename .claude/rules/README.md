...
```
                    ┌─ mindset.md (上位心智层)
                    │  三唯一框架 / P-J-A / 用户意图→Skill 路由表
                    │
                    ├─ handoff.md (交接协议)
    ┌── task.md       │  启动仪式/回交模型/Agent约束/Plan→Dtask
    │  状态机         │
    │                 ├─ testing.md (测试分层)
    │                 │  幅度→验证方式/补测试触发
    │                 │
    ├─ session.md      ├─ verification.md (证据体系)
    │  Session 结束    │  L1-L5/Done判定/无法验证处理
    │                  │
    ├─ templates.md    └─ templates.md (格式模板)
    │  模板             BLOCKED/REVIEW/DECISION TRACE/Session/Commit
    │
    ├─ pitfalls.md       ├─ exceptions.md
    │  误判表            │  阻塞判定
...
| 做判断决策 | `mindset.md` §现象→判断→动作 / `pitfalls.md` | 三段论骨架 + 高频误判表 |
...
| 文件 | 行为 | 对应 Skill | 何时查阅 |
|------|------|-----------|---------|
| **mindset.md** | 上位心智层：三唯一框架、不确定性门控、用户意图→Skill 路由表 | *核心层内嵌* | Session 启动、开工检查时 |
...
| **session.md** | Session 结束规范：时间戳+踩坑+3-Strike+Checkpoint | dsess + drecord | Session 开始、结束时 |
| **verification.md** | 证据优先级体系：L1-L5、Done 判定门槛、无法验证处理 | dtask + drun + dcorr | 选择证据等级、判定完成标准时 |
| **pitfalls.md** | 误判防护：Layer 2 项目高频表机制 / Layer 3 接口预留 | dcorr | SessionStart 注入、归档聚合时 |
| **exceptions.md** | 异常处理与 BLOCKED 判定、阻塞恢复流程 | *参考文件* | 遇到阻塞、需要人工介入时 |
| **templates.md** | 格式模板：BLOCKED/REVIEW/DECISION TRACE/Session/Commit/Checkpoint | drecord | 输出标准格式时 |
| **constraints.md** | 架构约束：六维约束 + 规则回写约束 | *参考文件* | 设计新功能、定义约束时 |
| **file-layout.md** | 目标项目文件放置规则：.diwu/ / tests/ / rules/ | *参考文件* | 了解文件组织、归档、查历史 |
...
2. `task.md` → 掌握任务语言和状态机
4. `verification.md` + `templates.md` → 掌握验收标准

### 日常使用

- 遇到具体问题 → 按上表"按场景导航"直达目标文件
- 实施中遇到判断困境 → `mindset.md` §现象→判断→动作 + `pitfalls.md` 高频表
