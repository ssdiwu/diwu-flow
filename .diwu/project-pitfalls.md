...
## Source: archive-aggregate-2026-05-10
- [分层未拆清] Edit 操作反复被外部恢复问题（dloop.py 遇到 3 次 Edit 失败）→ 最终用 Write 整文件重写解决。疑似 pytest 缓存或 hook 监控导致，根因未完全确认。 （来源: session-2026-05-09-225204.md）- [环境漂移] `test_start_and_status_use_same_state_path` 返回 invalid_state_file → 根因是 `_normalize_loop()` 仍强制要求 active dloop 含 `session_id` 字段（session 模式残留验证）。修复：移除 session_id 必填校验，mode 默认值改为 "cron"。 （来源: session-2026-05-09-225204.md）- [验证误读] `test_stop_decision_inprogress_always_blocks` 第二次调用返回空 stdout → 根因是 dloop state 存在时走 `decide_cron_mode()` 路径，而该函数不检查 InProgress 任务。修复：在 `decide_cron_mode()` 入口加入与 `decide_default_mode()` 一致的 InProgress 阻断逻辑。 （来源: session-2026-05-09-225204.md）
## Source: archive-aggregate-2026-05-10
- [其他] 发版脚本清理列表维护遗漏 → 新出现的本地工具目录（.playwright-mcp）未加入清理范围 → 误以为是仓库本身问题 → 发版脚本应作为敏感目录清单的唯一下游消费点，任何新增本地工具目录都应同步审查是否需要排除; 公开仓库 tag 更新时 annotated tag 的 SHA 与指向 commit SHA 混淆 → git ls-remote 看到的是 tag object SHA（a90d075），不是 commit SHA（f6fdf05） → 需要 `git cat-file -p` 确认指向关系，避免误判更新失败 （来源: session-2026-05-10-003653.md）- [其他] CHANGELOG 放发版清单导致回忆式填写易遗漏 → 根因是职责错位，持续追加属于变更传播而非发版检查 → 各归其位：CHANGELOG 追加移入变更传播，发版清单改为最终确认态 （来源: session-2026-05-10-014219.md）- [分层未拆清] ddoc 定位漂移：当前 CLAUDE.md 将 ddoc 归入 L1 思考收束层，但其实际价值在实施闭环（drun→ddoc 同步文档）。 → ddoc 移至 L5 实施层，与 dtask/drun 并列。 （来源: session-2026-05-10-025418.md）- [数据缺口] didea 产出了一个描述仅含"ddd"的空壳 idea 文件 → 根因是 create 动作缺乏正文最低门控，AI 不追问直接写文件 → 产物完全违背 didea"挂住灵感"的设计初衷 → 追加规则 3 正文最低内容门槛 + 软入口引导追问三句 （来源: session-2026-05-10-014219.md）- [数据缺口] 当前 type 三值（product/rule/tool）混合了三种不同的分类维度：product 按思维层级、tool 按操作方式、rule 按文档性质。根因：type 是事后贴标签而非事前设计。 → 删除 type，分类信息保留在 skills/README.md 分组表中。 （来源: session-2026-05-10-025418.md）- [读层现象] 联网搜索资料（AltexSoft 等）推荐的"AI 适合度标注"反映上一代人工文档思维（文档逐渐腐化）。 → 不采纳，AI 时代应做到每版本同步更新。 （来源: session-2026-05-10-025418.md）
## Source: archive-aggregate-2026-05-10
- [其他] 发版时校验 vs 测试时校验 → 用户正确指出版本号一致性应左移到测试层（pytest每次都跑），而非拖到发版时刻才暴露 → 正确做法：新增 level3 一致性测试，drelease.sh 保持简洁 （来源: session-2026-05-10-031546.md）- [环境漂移] macOS grep 不支持 `-P` (Perl regex) → 在 drelease.sh 校验逻辑中用 `grep -oP` 提取 install.sh 版本号时失败 → 改用 Python re 或 `grep -E`；最终因方案调整未采用此路径，但记录 macOS grep 差异 （来源: session-2026-05-10-031546.md）
## Source: archive-aggregate-2026-05-10
- [分层未拆清] Cv2 hook 注入方案被否决的根因与 dloop cron 迁移是同一个问题（session owner 不可靠）→ 但表现形式不同（Cv2 是"per-session 上下文注入"，dloop 是"任务归属判断"）。教训：同一根因会在不同方案中以不同面具出现，需要识别本质而非处理表象 （来源: session-2026-05-10-034556.md）- [读层现象] triggers 数组看似结构化完善，实际对自然对话召回率仅 10% → 根因：triggers 捕捉指令式表达（"先记下来"），真实对话是发散式表达（"聊聊这个"、"我觉得可以这样"）。正确做法：用语义意图检测替代关键词匹配（/diwu 元 skill L0 层） （来源: session-2026-05-10-034556.md）- [路由护栏契约] didea 作为 L0 入口在本次 session 完全沉默（7+ 次应触发零命中）→ 暴露的不是单个 skill 问题而是整个触发模型假设缺陷（"用户会主动用 command/skill" vs "用户只是自然聊天"）。这直接推动了 /diwu 元 skill 从"可选增强"升级为"核心基础设施" （来源: session-2026-05-10-034556.md）- [验证误读] superpowers 的 1% 规则不能直接搬用到 diwu-flow → 误判为通用最佳实践。实际差异：superpowers 误触代价低（多读 frontmatter），diwu-flow 误触代价高（可能触发完整方法论执行干扰工作流）。修正：引入 SHOULD 档位替代 MUST，保留 AI 最终裁决权 （来源: session-2026-05-10-034556.md）
## Source: archive-aggregate-2026-05-10
- [分层未拆清] rules/ 和 skills/ 的厚度边界是在本次 session 中逐步清晰的（"rules 做薄定性守卫、skills 做厚渐进式披露"），但现有 14 个 rule 文件不符合此原则——judgments.md 200行纯索引、task.md 混入实现细节、workflow.md 被 mindset 完全覆盖。教训：确立原则后应立即审计存量，否则原则变成空话 （来源: session-2026-05-10-040330.md）- [读层现象] 把"在哪个 issue 里产生的"当作"文件归属依据 → 错误分类。正确做法是"谁消费、什么场景"。导致差点把 skill-routing.md 归类为"插件专属不该进 rules/"，实际上它是所有目标项目 AI 都需要的技能选择协议 （来源: session-2026-05-10-040330.md）- [路由护栏契约] 同一个根因（session owner 不可靠）以三种不同面具出现：dloop cron 迁移、Cv2 hook 注入被否决、hooks 多窗口隔离担忧。教训：识别本质模式而非处理表象；同一根因会在不同方案中以不同面具重复出现 （来源: session-2026-05-10-040330.md）- [验证误读] superpowers 的 1% MUST 规则不能直接搬用 → 误判为通用最佳实践。修正为 SHOULD + 保留 AI 最终裁决权。更深层教训：任何外部模式的参数都不能直接抄，必须结合自身约束条件（diwu-flow 误触代价 > superpowers）调整 （来源: session-2026-05-10-040330.md）
## Source: archive-aggregate-2026-05-10
- [分层未拆清] L-Rule vs L-Skill 的职责边界在讨论过程中逐步清晰 → 最终锁定"L-Rule=自动守卫(每条回复前), L-Skill=路由顾问(不确定时深度分析)"。教训：双轨设计的核心不是"两个都做"，而是明确"什么场景下用哪个"——如果边界模糊就会变成重复劳动或互相推诿。L-Rule 解决"别漏掉"(90%消息)，L-Skill 解决"不确定时问一下"(10%边界 case) （来源: session-2026-05-10-044857.md）- [分层未拆清] Issue 讨论和 Issue 原始 body 是两个不同阶段的信息资产。body = 初始提案（可能已被推翻/超越/细化），comments = 演进后的当前真相。把 body 当作 Issue 当前状态的代理 → 必然出错。正确优先级：comments > body > title （来源: session-2026-05-10-045846.md）- [读层现象] 把"AI 能理解复杂指令"等同于"AI 会可靠执行复杂流程" → 错误。设计了四维打分引擎（D6 初版），形式完备但执行时 AI 更可能倒推填分数而非真正逐项评估。这与 triggers 10% 召回率是同一个根因：**结构化匹配/评分流程对 AI 来说不是自然行为，查表和语义直觉才是**。修正：L-Skill 从"评分引擎"降级为"路由参考卡"——信号速查卡 + 文本匹配，让 AI 做它擅长的事（模式匹配）而不是强迫它做它不擅长的事（结构化评分） （来源: session-2026-05-10-044857.md）- [读层现象] **分析 Issue 时只读了原始 body，未读取后续评论就给出分析和建议** → 导致对 #29 的 D10 分析完全基于过时信息（原始 body 的 A/B/C/D 四方向），而 #29 实际上已经经过 5 轮讨论、7 条评论、演化为完整五层模型（L0 三特征 AND 检测 + Q1 定性三态去重 + Q2 压缩标题 + 时间衰减 lifecycle），且 10/10 未定点已全部清零。浪费了用户时间做了一次已完成的决策讨论。**正确做法：分析任何 Issue 前，必须先读完整评论链（尤其是 `--comments` 全量输出），以最新状态为准；原始 body 只是起点不是终点** （来源: session-2026-05-10-045846.md）- [路由护栏契约] 同一根因以不同面具第三次出现：triggers 10% 召回(design缺陷) → didea 零触发(同上) → 四维打分引擎不可靠(同上)。本质都是"期望 AI 按预定义流程执行检测/匹配/评分"。教训：这个根因会在每次设计"AI 自动执行 X"的方案时重复出现，识别后应立即检查：这个流程 AI 会天然执行还是需要强制？ （来源: session-2026-05-10-044857.md）- [路由护栏契约] 本错误的更深层模式：我在 D10 分析中自信地给出了方向推荐（C 为基准/A 为 fallback）并设计了交互流程——但这一切都建立在"不知道 #29 已有完整规格"的前提下。**当基础事实缺失时，任何精致的分析都是空中楼阁，且越精致越误导（因为看起来很专业会增加可信度权重）**。防御措施：对任何 Issue 给出具体建议前，先确认"我是否已读过该 Issue 的完整讨论历史" （来源: session-2026-05-10-045846.md）- [验证误读] XML tag (`<routing-check>`) 假设有机器可读语义 → 经 explorer Agent 实证调查确认 Claude Code 对 XML tag 无任何特殊处理。优先级由物理位置决定（用户级 > 项目级 > hook 注入位置），不由标记格式决定。教训：任何关于平台行为的假设必须验证，不能从其他系统（如 HTML/XML 解析器）类推 （来源: session-2026-05-10-044857.md）- [验证误读] 同一根因第四次面具：triggers 10% 召回(design 缺陷) → didea 零触发(同上) → 打分引擎不可靠(同上) → **Issue 评论不同步(本次)**。本质相同：依赖过时/不完整的信息源做决策。教训扩展：不仅是 AI 执行流程不可靠，**"信息源是否为最新版本"本身就是一个必须检查的前置步骤** （来源: session-2026-05-10-045846.md）
## Source: archive-aggregate-2026-05-11
- [分层未拆清] 分析 dref 是否需要多视角时，第一反应是"和 dpth 一样加双模"→ 但拆解原子能力后发现 dref 的核心 R4（苏格拉底式提问）和多视角判断是完全不同的认知模式。多视角=同一问题不同角色各看一遍（适合判断类），苏格拉底法=引导用户逐步暴露隐含假设（适合收敛类）。前者受益于多视角，后者会被多视角破坏（三个 explorer 同时提问造成信息过载）。教训：**给技能加能力前必须先拆解其原子能力，看每个原子是否真的需要增强，而不是因为"隔壁加了所以我也要加"** （来源: session-2026-05-10-135521.md）- [读层现象] dref 的 description 写着"先判断问题真假，再收敛为可执行清单"——"先判断真假"这半句暗示 dref 有独立判断职责，但实际上这个职责完全被 dpth 覆盖了（且 dpth Active 更强）。description 的措辞导致了功能越位的错觉。教训：**skill description 是契约声明，不是能力愿望清单；写了"我能做 X"就意味着使用者会期望你做 X，即使 X 不是你的核心价值** （来源: session-2026-05-10-135521.md）- [路由护栏契约] 同一根因第五次面具出现：triggers 10% 召回(design 缺陷) → didea 零触发(同上) → 打分引擎不可靠(同上) → Issue 评论不同步(信息源过时) → **技能功能越位(description 声称了不属于它的能力)(本次)**。本质始终是同一个："声称能做的事"和"实际该做的事"之间的错位。防御措施扩展：不仅检查 AI 执行流程可靠性，还要检查 skill 自身的能力边界声明是否准确 （来源: session-2026-05-10-135521.md）