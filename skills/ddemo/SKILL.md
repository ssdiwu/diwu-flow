...
version: "1.0"
type: rule
description: "积木式能力验证方法论——判断「直接做 vs 先验证」、将不确定性分层隔离、沉淀可复用能力资产"
triggers:
  - "讨论技术可行性或方案选型"
  - "评估某个实现的不确定性"
  - "决定直接集成 vs 先做 Demo 验证"
  - "分析能力复用或知识沉淀策略"
  - "用户说 积木、Demo、能力验证、不确定性"
keywords:
  - "Demo"
  - "能力验证"
  - "不确定性"
...
1. **业务约束编码**：「原始录音不可变，摘要可重新生成」
   - 编码方式：raw_transcripts 表无 updated_at 字段，cards 表的内容是投影
   - 验证方法：删除 cards 表数据，能从 raw_transcripts 重算

2. **时序约束编码」：「停止录音后同一 session 不可恢复」
   - 编码方式：状态机转移表中 Stopped 状态无到 Recording 的转移
   - 验证方法：单元测试覆盖所有非法转移，确认返回错误或忽略

3. **并发约束编码**：「状态机事件串行处理」
   - 编码方式：所有事件通过 Channel 串行化，transition 函数是纯函数
   - 验证方法：并发测试发送 100 个事件，状态转移顺序正确

### 何时使用

- **能力验证阶段**：识别核心约束，在 Demo 中验证约束可编码
- **集成阶段**：将约束从 Demo 迁移到产品代码，确保不被绕过
- **Code Review**：检查实现是否违反已声明的约束
