...
version: "1.0"
type: rule
description: "阶段边界决策锚点——四段式判断（启动/实施/验收/纠偏），含正例/反例/边界例。覆盖：基线失败处理、不确定性决策、入口门控、大幅度判定、执行偏差分级、并行串行选择、超前实施、Done 人工确认、blocked_by 判定、循环依赖、continuous_mode 续跑、recording 写入时机、decisions.md 写入时机"
triggers:
  - "需要做阶段边界决策"
  - "判断幅度大小"
  - "选择并行还是串行"
