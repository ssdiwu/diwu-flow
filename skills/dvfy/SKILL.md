...
version: "1.0"
type: rule
description: "验证证据优先级体系——L1-L5 五级证据、Done 判定门槛矩阵、完成前四问、无法验证处理规范、运行态验证方法指引、独立验证"
triggers:
  - "标记 InReview 或 Done"
  - "选择证据等级"
  - "判断是否验证充分"
...
当需要独立第三方验证时（而非自审），按以下流程：

1. 从 acceptance 反推可观测事实（不信任实现者的自述）
2. 逐条检查代码变更是否匹配 acceptance 要求
3. 对不确定输出 HUMAN_NEEDED
4. 输出验证报告：PASS/FAIL/HUMAN_NEEDED 逐条标注
