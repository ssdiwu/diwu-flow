...
- `/dtask` 只负责生成/改写 `.diwu/dtask.toml` 中的任务定义，默认写入状态始终是 `InDraft`
- `dtask.toml` 是任务内容与 `status` 的真相源
- `.diwu/dtask-state.toml` 是运行态 owner / dloop 元数据真相源
...
- 参数："分开，你可以用两个子代理完成任务"
- 结论：提取上下文功能描述（修改 ddoc.md + 修改 dprd.md），直接进入 Step 1.5

反例（参数是功能描述 → 直接进入 Step 1.5）：
- 上下文：无特定待回答问题
- 参数："修改 ddoc.md 和 dprd.md，增加工具链联动"
...
说明：`/absolute/path/to/project` 为占位路径，需替换为真实绝对路径。

思维粒度：`title` 一句话、`acceptance` Given 具体路径/函数、Then 可断言、`steps` 真实绝对路径。
...
- **边界**：明确不做什么？
- **成功标准**：怎么算完成？
- **具体例子**：典型操作的完整例子（推导 Given/When/Then）
...
#   降级策略：手动读取 dtask.toml + archive/ 获取 max_id
...
# → 输出: {"ok": true, "max_id": N, "source": "dtask.toml|archive|empty"}
```
...
根据澄清结果生成任务列表，追加到 `.diwu/dtask.toml`。
...
边界情况：
- `.diwu/dtask.toml` 不存在：创建 `{"tasks": []}` 再追加
- `.diwu/` 目录不存在：先创建目录
...
从三个视角审查，按子代理策略（见 `rules/handoff.md` §Handoff Matrix）派发并行审查：
