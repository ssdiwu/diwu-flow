...
- 包含 `reverse`、`逆向`、`还原文档`、`从代码` → 自动设为**逆向模式**，跳过 Step 1 的模式询问
- 包含 `forward`、`正向`、`写文档`、`从需求` → 自动设为**正向模式**，跳过 Step 1 的模式询问
- 均未匹配 → 进入 Step 1 询问
...
读取代码库，按四层结构写文档。大范围时，按子代理策略（见 workflow.md）对各域并行探索，每个域的子代理只写自己的草稿文件，主代理汇总草稿后执行 gap 分析和跨域一致性检查：
...
> **文件操作安全（R1）**：修改已有文件前先 Read 当前内容；整文件重写先 Read 完整文件；新建文件确认不存在后再 Write。写入文档前**必须先 Read 目标文件当前状态**，确认不会意外覆盖。

### 领域驱动结构
```
.doc/
├── README.md          # 索引：文件一览 + 每个文件一句话说明覆盖范围
...
| 文件 | 覆盖范围（一句话） | 最后更新 |
|------|-----------------|---------|
| constraints.md | 全局跨域约束，包含并发、幂等、超时策略 | 2026-02-26 |
| auth.md | 认证域：JWT 结构、登录流、权限模型 | 2026-02-26 |
| message.md | 消息域：实体、状态机、推送链路 | 2026-02-26 |
```

列格式定义见 `.doc/schema.md` §Doc。

**首次创建 README（兼容旧版本）**：glob 扫描所有 .md（排除 README），读取首行标题+修改时间生成索引表，再追加当前文档。

**日常维护**：
写入或更新任意文档文件后，同步更新 README.md 对应行（覆盖范围描述 + 最后更新日期）。

### 其他命令读取本目录时的入口规范

`/dprd`、`/ddemo` 等命令在需要了解现有系统能力时：
1. 只读 `.doc/README.md`，根据覆盖范围描述判断需要精读哪几个文件
2. 定向读取相关文件，不全量扫描

**判断锚点：定向读取 vs 全量扫描**

- 正例（定向读取）：用户要做"AI 回复引用会议"功能，README 中有"message.md（消息域：实体、状态机、推送链路）"和"auth.md（认证域：JWT、登录流）"。结论：只读 message.md，auth.md 无关跳过。
- 反例（全量扫描）：README 不存在或为空。结论：glob 扫描 `.doc/` 全部文件。
- 边界例（部分定向）：用户要做"带权限的消息推送"，README 中有 message.md 和 auth.md。结论：两个都相关，定向读取这两个文件，其余跳过。
