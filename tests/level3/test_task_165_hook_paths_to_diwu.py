...
TASK_JSON_PATH = PROJECT_ROOT / ".claude" / "dtask.toml"
...
EXPECTED_DIWU_FILES = {
    ...
    "session_start.py": [],
    "task_created_validate.py": [".diwu/dtask.toml"],
    # 库模块（被 stop_decision.py 内部 import 调用，非独立 hook 入口）
    "stop_archive.py": [".diwu/dtask.toml", ".diwu/dsettings.toml", ".diwu/recording"],
    # 以下为规划中/未实现：
...
def test_task_created_validate_uses_relative_diwu_task_path():
    ...
    assert 'TASK_JSON_PATH = ".diwu/dtask.toml"' in text
