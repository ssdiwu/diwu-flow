...
| verification.md 等级 | testing.md 验证方式 | 使用场景 |
|---------------------|-------------------|---------|
| L1 运行态 | 运行态验证（`[建议]`） | 关键链路是否真的经过新代码 |
| L2 调用链 | 运行态验证（`[建议]`） | 底层状态是否真正切换 |
| L3 自动化断言 | 自动化验证（推荐） | 单元测试、verify.sh、pytest |
| L4 表面观察 | 手动验证 | UI 交互、实机测试（Agent 提供步骤→人工反馈） |
| L5 间接推断 | — | 不作为独立验收依据 |

> 完整的幅度→验证方式映射和补测试触发条件见 `rules/testing.md`。
> InReview→Done 场景化判定锚点见 `rules/task.md` §状态转移判定锚点。
