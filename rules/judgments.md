...
> 规则见 `rules/task.md` §Architect 审稿 Gate 和 `agents/architect.md`。
...
> 规则见 `agents/debugger.md` 和 `skills/drun/SKILL.md` §Debugger 异常路由。

| 场景 | 判定 | 依据 |
|------|------|------|
| 正例 | 异常排查场景（bug/行为不符/工具失败 3-Strike）直接优先走 debugger | debugger 诊断效率高于 explorer 通用探索 |
| 反例 | 首次接触代码库仍走 explorer | 无异常信号，不需要诊断专才 |
| 边界 | 工具偶发失败 1-2 次不走 debugger（正常重试范围）；debugger 不替 dcorr 做方法论纠偏 | 以是否有明确异常信号和重复失败模式为界 |
